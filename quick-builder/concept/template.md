...
Quick Builder를 통해 ICS에 템플릿을 일괄 등록하기 위해, Template을 아래와 같이 준비가 필요합니다. Template 내의 링크 및 리소스의 경로, 폴더 및 공통 영역에 대한 소스 등을 사전에 정의해두면 빠르게 쉽게 ICS를 위한 템플릿이 등록됩니다.
...
Resource File의 네이밍이 ICS의 규격과 맞지 않는 경우 Quick Builder로 등록이 불가할 수 있습니다.  파일내에 아래와 같은 파일명이 있다면, 다른 네이밍으로 변환하여 저장해주세요.

{% hint style="info" %}
Resource file에서 지원하지 않는 파일 네임&#x20;

특수문자(\~!@#$%^&\*()-+), Space를 사용하지 마세요.&#x20;
{% endhint %}
...
디렉토리 구성Template 내 생성한 디렉토리는 향후 사이트 카테고리가 됩다.&#x20;
...
> **주의사항 **
>
> 메뉴의 뎁스는 최대 9뎁스 이상을 초과하면 안됩니다.
>
> 한 뎁스 내에 생성될 폴더는 최대 1156개를 초과하면 안됩니다.



### Link 설정&#x20;

local에서 열어보더라도 온전하게 링크가 되도록 파일간의 경로는 “/” 또는 “../” 등의 절대경로, 상대경로로 링크가 되어야 합니다.&#x20;
...
파일은 템플릿으로 등록할 파일과 리소스 업로드로 등록할 파일이 나눠지는데, 연결 시, 리소스 파일로 링크가 되는 부분은 \[\[--ImgRootDir--]]로 변환되며, 템플릿으로 링크가 되는 부분은 \[\[--CatAddress--]]에 옵션이 추가된 상태로 변환됩니다.

만약, Link 를 상대경로로 처리하고, Template만 Zip으로 압축할 경우, 상대경로 상에 실제로 링크될 파일이 있다면, Template의 경로도 변환됩니다.&#x20;
